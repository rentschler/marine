...
export const CommunitySelector = memo(
  ({
    selectedCommunities,
    ...
    const isAllSelected = useMemo(() => {
      return (
        communities && communities.length > 0 && selectedCommunities.length === communities.length
      );
      ...
      <div className="flex items-center gap-2">
        <div className="flex">
          ...
          {isAllSelected && showCount && (
            <span className="text-xs text-gray-500">All communities selected</span>
          )}
        </div>
      </div>
    );
