...
export function initScene(
  ...
  if (!cameraRef.current) {
    ...
    (cameraRef.current as THREE.PerspectiveCamera).aspect = containerWidth / containerHeight;
  ...
  if (!rendererRef.current) {
    ...
    currentContainer.appendChild((rendererRef.current as THREE.WebGLRenderer).domElement);
  } else {
    (rendererRef.current as THREE.WebGLRenderer).setSize(containerWidth, containerHeight);
  }
