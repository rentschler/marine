    ...
    try:
        ...
        async with AsyncGraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD)) as driver:
                async with driver.session() as session:
                    ...
                    result = parsed_answer.model_dump(exclude_unset=True, exclude_none=True)
...
async def test_query(question: str):
    kr = KnowleadgeGraphRetriver(llm=llm, index_summary_path="retriver/index/retriever_summary.json")
    return await kr.extract_query_params_from_question(question=question)